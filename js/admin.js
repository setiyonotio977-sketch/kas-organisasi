...
async function loadAll(){try{[data.finance,data.members,data.structure,data.news,data.agendas,data.albums,data.messages]=await Promise.all([getCol('finance','date'),getCol('members'),getCol('structure','order'),getCol('news','date'),getCol('agendas','date'),getCol('albums'),getCol('messages','createdAt')]);console.log('DEBUG MEMBERS DARI FIRESTORE:',data.members);renderAll()}catch(e){console.error('DEBUG LOAD ERROR:',e);alert('Gagal memuat data. Pastikan Firebase config dan Security Rules sudah benar.\\n'+e.message)}}
